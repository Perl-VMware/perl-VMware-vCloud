...
sub org_vdc_update {
    ...
    my $alloc =
        $conf->{allocation_model} ? '<AllocationModel>' . $conf->{allocation_model} . "</AllocationModel>\n" : '';
    my $compute;
    ...
    my $pvdc =
        $conf->{pvdc_href}
        ? '<ProviderVdcReference name="' . $conf->{pvdc_name} . '" href="' . $conf->{pvdc_href} . "\" />\n"
        : '';
    my $fast =
...
* Fencemode can be: bridged, isolated, or natRouted * IP Allocation mode can
be: NONE, MANUAL, POOL, DHCP
